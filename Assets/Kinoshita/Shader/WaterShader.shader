            ...
            output frag(const v2f i)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                   ...
                   if (dist < 0.2)
                   {
                        ...
                        const fixed3 rimColor = fixed3(0.8, 0.8, 1.0); // 輪郭の色

                        float highlight = dot(norm, -rayDir) > 0.99 ? 0.5 : 0; // ハイライト
                        fixed3 color = clamp(lerp(baseColor, rimColor, rimRate) + highlight, 0, 1); // 色
                        float alpha = clamp(lerp(0.2, 4, rimRate) + highlight, 0.8, 1); // 不透明度
